# Paxos Made Simple
Paxos is a consensus algorithm, as you'd already have known. Simply put, given a set of processes generating some values, the system will choose *only one* value and will remember that chosen value.
...
- We're following an *asynchronous, non-Byzantine* model. Heavy emphasis on non-Byzantine.
...
Given **2n + 1** acceptors the majority is **n + 1**, and we can tolerate up to **n** failures. An even count works mathematically too — with **2n** acceptors the majority is still **n + 1** — but it tolerates only **n − 1** failures, so we'd be paying for an extra node without buying any extra fault tolerance. So we stick with odd. As long as **n + 1** servers are online, mathematically at least one of those **n + 1** servers is guaranteed to have been part of the prior majority (any two majorities of **n + 1** drawn from **2n + 1** must overlap, since 2(n + 1) > 2n + 1).
...
We'll now go through a couple of scenarios to see how the proposals can flow and what happens to our system given this single guideline, and that the acceptors accept only the first proposal that reaches them.
...
Now one more proposal Pb with value v2 is issued. According to our updated constraints the acceptors can accept this proposal, and say 3 of the acceptors accept this new proposal. Now the chosen value is v2. This violates one of our guarantees that once a value is chosen it won't be changed.

<img src="../assets/multiple_ack.png" width="600">

To mitigate this we need some mechanism that would let the system remember that a value has indeed been chosen. The most common choice that comes to mind is that after a consensus is reached the acceptors should no longer accept any values. Paxos doesn't go this route, it mandates that `once a value is chosen, all further proposals must be made with the chosen value`. The proposer doesn't *know* the chosen value directly — the prepare phase (covered next) forces it to discover any previously-accepted value from the acceptors and adopt the highest-numbered one before issuing its own accept. The non-Byzantine assumption is what makes this safe: everyone follows the protocol honestly. A malicious proposer that ignored the prepare-phase response could propose anything it likes and break the algorithm.

To achieve this Paxos numbers each proposal. The proposal numbers are monotonically increasing and drive the core of the algorithm.
...
#### Prepare Phase
During this phase 
- The proposer will send a prepare request to all the acceptors and try to extract a promise from the acceptors stating that the acceptor will no longer *accept proposals numbered less than the proposed proposal*. 
- The acceptor will respond positively to the prepare request only if it hasn't responded to a prior prepare request with higher number. 
	...
	- If the acceptor accepts the prepare request it responds with two things:
		1. A promise to not accept proposals numbered less than the currently held proposal.
		2. The highest proposal that the acceptor had accepted prior to this and its value, if it had accepted one already. Returns nothing otherwise.
...
- The proposer now sends an accept request carrying the proposal_number, and value to all of the acceptors. (Since it passed the prepare phase the majority has agreed and we're not constrained to sending to only the acceptors that accepted the prepare request)
	- On receiving the accept request, an acceptor accepts it *unless* it has, in the meantime, *promised some prepare with a higher number*. (Between giving its original promise and now, another proposer may have come along with a higher number — in which case the original promise has been superseded and this accept is rejected.)
